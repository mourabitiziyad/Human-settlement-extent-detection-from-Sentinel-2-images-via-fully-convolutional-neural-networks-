    ...
    def Bands2patches(self, imgMat, upSampleR):
        ...
        for band in np.arange(imgMat.shape[2]):
            ...
            if upSampleR != 1:
                ...
                arr = scipy.ndimage.zoom(arr, [upSampleR,  upSampleR], order=1)
            ...
            if band == 0:
                ...
                patch = np.zeros(
                    ((patch0.shape[0]-idxNan.shape[0]), self.dim_x, self.dim_y, imgMat.shape[2]), dtype=imgMat.dtype)
...
# #from a multi bands mat to patches, without considering the nan area
    ...
    def Bands2patches_all(self, imgMat, upSampleR):
        for band in np.arange(imgMat.shape[2]):
            ...
            if band == 0:
                patch = np.zeros(
                    ((patch0.shape[0]), self.dim_x, self.dim_y, imgMat.shape[2]), dtype=imgMat.dtype)
...
# output:
    ...
    def __img2patch(self, mat):
        ...
        # mat=np.pad(mat, ((int(self.dim_x_img/2), int(self.dim_x_img/2)), (int(self.dim_y_img/2), int(self.dim_y_img/2))), 'reflect')

        window_shape = (self.dim_x, self.dim_y)  # self.dim_x_img

        # window_shape = (self.dim_x, self.dim_y)#self.dim_x_img
        # B = view_as_windows(A, window_shape,2)
  ...
  '''
    ...
    def predic2tif(self, mat, prj, trans, mapFile):
        ...
        if self.isSeg == 1:
            xres = trans[1]*1
            yres = trans[5]*1
            geotransform = (trans[0]+xres*(1-1)/2.0, xres,
                            0, trans[3]+xres*(1-1)/2.0, 0, yres)
            print(geotransform)
        else:
            ...
            geotransform = (trans[0] + trans[1]*(self.dim_x-1)/2.0, xres, 0,
                            trans[3] + trans[5]*(self.dim_y-1)/2.0, 0, yres)  # no padding
       ...
       # create the dimZ raster file
        # gdal.GDT_Byte .GDT_Float32 gdal.GDT_Float32
        dst_ds = gdal.GetDriverByName('GTiff').Create(
            mapFile, C, R, dimZ, gdal.GDT_UInt16)
        dst_ds.SetGeoTransform(geotransform)  # specify coords
        dst_ds.SetProjection(prj)

        for i in np.arange(dimZ):
            map = mat[:, :, i]
            # write band to the raster
            dst_ds.GetRasterBand(int(i+1)).WriteArray(map)
    ...
    def scaleBand(self, patches):
        ...
        patches_ = scaler.transform(patch).reshape(
    ...
    def pro_from_x(self, mapR, mapC, y, padding):
        ...
        C = np.zeros((B_.shape[0]*B_.shape[2], B_.shape[1]
                     * B_.shape[3], B_.shape[4]), dtype=float)
        for dim in np.arange(B_.shape[4]):
            B_1 = B_[:, :, :, :, dim]
            C[:, :, dim] = B_1.transpose(
                0, 2, 1, 3).reshape(-1, B_1.shape[1]*B_1.shape[3])
            del B_1
